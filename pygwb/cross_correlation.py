...
def cross_correlation(
    ...
    for II in range(numIntervalsTotal):
        ...
        d1_mid = slice_time_series(
            d1, offset + length_of_segment, offset + 2 * length_of_segment
        )
        d1_right = slice_time_series(
            d1, offset + 2 * length_of_segment, offset + 3 * length_of_segment
        )

        d2_left = slice_time_series(d2, offset, offset + length_of_segment)
        d2_mid = slice_time_series(
            d2, offset + length_of_segment, offset + 2 * length_of_segment
        )
        d2_right = slice_time_series(
            d2, offset + 2 * length_of_segment, offset + 3 * length_of_segment
        )
        ...
        c = FrequencySeries(
            d1_mid_tilde.freqs, d1_mid_tilde.data * np.conj(d2_mid_tilde.data)
        )
        ...
        Y_f = FrequencySeries(
            P1.freqs, 2 * c_cg.data / (segmentDuration * orf * S0.data)
        )
        Y_f = Y_f / w1w2bar
        var_f = FrequencySeries(
            P1.freqs,
            (P1.data * P2.data)
            / (2 * segmentDuration * deltaF)
            * orf ** (-2)
            * S0.data ** (-2),
        )
        ...
        if verbose:
            print(
                "stochastic: Done with Interval %u / %u" % (II + 1, numIntervalsTotal)
            )
            print(f"\tY     = {Ys[II]:e}")
            print(f"\tsigma = {sigs[II]:e}")
            print(f"\tSNR   = {Ys[II] / sigs[II]:f}")
