...
class StatisticalChecks:
    ...
    def plot_hist_coherence(self,total_bins = None):
        ...
        coherence_highres = np.linspace(min(coherence), max(coherence),num = 2*total_bins)
        ...
        axs.set_ylim(10**(np.floor(np.log10(self.coherence_pdf(threshold)))-1), 10**(np.floor(np.log10(predicted_highres[0])) + 1))
