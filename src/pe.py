...
class BrokenPowerLawGWBLikelihood(GWBLikelihood):
    ...
    def __init__(self,baselines,fref):
        super().__init__(baselines,parameters={'A':None,'alpha1':None,'alpha2':None,'fbreak':None})
        self.fref=fref
    ...
    def OmegaGW(self,freqs):
        ...
        if fref <= fbreak:
            ...
            Omega[freqs>fbreak]  = A * (fbreak/fref)**(alpha1-alpha2) * (freqs[freqs>fbreak]/fref)**alpha2       
        else:
            Omega[freqs<=fbreak] = A * (fbreak/fref)**(alpha2-alpha1) * (freqs[freqs<=fbreak]/fref)**alpha1 
...
class SVTPowerLawGWBLikelihood(GWBLikelihood):
    ...
    def __init__(self,baselines,fref=1):
        super().__init__(baselines,parameters={'AT':None,'alphaT':None,
                                               'AV':None,'alphaV':None,
                                               'AS':None,'alphaS':None})
        self.fref=fref
