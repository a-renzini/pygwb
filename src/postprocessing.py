...
def postprocessing(Ys,sigs,jobDuration,segmentDuration,deltaF,deltaT,bufferSecs=0):
    ...
    s_oe = np.sqrt( (w1w2squaredovlbar / w1w2squaredbar) * 0.5 *
                    (s_o**2 + s_e**2) *
                    (1.-1./(2*numSegmentsTotal)) )
...
def postprocessing_spectra(Y_fs,var_fs,jobDuration,segmentDuration,deltaF,deltaT,bufferSecs=0):
    M = int( np.floor( (jobDuration - 2*bufferSecs)/segmentDuration ) )
    numSegmentsTotal = 2*M-1
    ...
    v_f_oe =  ( (w1w2squaredovlbar / w1w2squaredbar) * 0.5 *
                    (v_f_o + v_f_e) *
                    (1.-1./(2*numSegmentsTotal)) )
