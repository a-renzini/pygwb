...
class TestInterferometer(unittest.TestCase):
    def setUp(self):
        ...
        with open("./detector_testdata_H1.pickle", "rb") as pkl:
    ...
    def test_set_timeseries_from_channel_name(self):
        ...
        self.assertTrue(ifo.timeseries.channel, self.kwargs["channel"])
        self.assertTrue(np.all(ifo.timeseries.value == self.testdata["filtered_timeseries"].value), True)
    ...
    def test_set_timeseries_from_timeseries_array(self):
        with self.assertRaises(AssertionError):
            ...
            ifo = detector.interferometer.get_empty_interferometer(self.ifo)
            ifo.set_timeseries_from_timeseries_array(self, timeseries_array, sample_rate, **self.kwargs)
        self.assertTrue(np.all(ifo.timeseries.value == self.testdata["filtered_timeseries"].value), True)
    ...
    def test_gwpy_timeseries_spectrogram_average_psd(self):
        with self.assertRaises(AssertionError):
            ...
            self.assertTrue(np.all(ifo.psd_spectrogram.value == self.testdata["psd_spectrogram"].value), True)
            ifo.set_average_psd(self.parameters.N_average_segments_welch_psd)
            self.assertTrue(np.all(ifo.average_psd.value == self.testdata["average_psd"].value), True)
