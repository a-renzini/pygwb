...
class TestSpectralDensities(unittest.TestCase):
    def setUp(self):
        ...
        data_start_time = 0
        self.time_series_data = timeseries.TimeSeries(data, t0=data_start_time, sample_rate=self.sample_rate)
    ...
    def test_csd(self):
        ...
        self.assertAlmostEqual(self.sigma, sigma_est, delta=0.1, msg="Injected and recovered parameters differ by more than 10%")
        
        # Check that the number of arrays is as expected
        expected_no_of_arrays = 2*self.no_of_segments-1 # only works for 50% overlapping
        self.assertEqual(expected_no_of_arrays, len(csd), msg="The csd array sizes do not match")
    ...
    def test_psd(self):
        ...
        self.assertAlmostEqual(self.sigma, sigma_est, delta=0.10, msg="Injected and recovered parameters differ by more than 10%")
        
        # Check that the number of arrays is as expected
        expected_no_of_arrays = 2*self.no_of_segments-1 # only works for 50% overlapping
        self.assertEqual(expected_no_of_arrays, len(psd), msg="The psd array sizes do not match")
        
        # Check whether the average PSD arrays is as expected
        expected_no_of_arrays = 2*(self.no_of_segments-2)-1 # only works for 50% overlapping
        self.assertEqual(expected_no_of_arrays, len(avg_psd), msg="The average psd array sizes do not match")
